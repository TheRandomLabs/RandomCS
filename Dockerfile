...
ENTRYPOINT $SERVER/update.sh $METAMOD_SOURCE_LINK $SOURCEMOD_LINK && \
		$SERVER/csgo.sh -console -usercon -port $PORT -tv_port $TV_PORT -authkey $WEB_API_KEY \
		-maxplayers_override $MAX_PLAYERS -net_port_try 1 +game_type 0 +game_mode 1 \
		+mapgroup mg_active +map de_dust2 +sv_password $PASSWORD +rcon_password $RCON_PASSWORD \
		+sv_setsteamaccount $GSLT +host_workshop_collection $WORKSHOP_COLLECTION \
